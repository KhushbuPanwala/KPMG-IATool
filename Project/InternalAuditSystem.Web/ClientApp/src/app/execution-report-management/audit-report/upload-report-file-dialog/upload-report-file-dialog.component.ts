...
import { ReviewerDocumentAC, ReportsService } from '../../../swaggerapi/AngularFiles';
...
@Component({
  selector: 'app-upload-report-file-dialog',
  templateUrl: './upload-report-file-dialog.component.html'
})
export class UploadReportFileDialogComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  reviewerDocumentObjList = [] as Array<ReviewerDocumentAC>;
  reviewerDocumentObj = {} as ReviewerDocumentAC;

  searchReviewerDocumentObjList = [] as Array<ReviewerDocumentAC>;
  ...
  constructor(private stringConstants: StringConstants, public bsModalRef: BsModalRef, private sharedService: SharedService,
              private loaderService: LoaderService, public uploadService: UploadService, private apiService: ReportsService,
  ...
  ngOnInit() {
    ...
    this.entitySubscribe = this.sharedService.selectedEntitySubject.subscribe((entityId) => {
      if (entityId !== '') {
        ...
        this.reportSharedService.updatedReviewerDocumentsSubject.subscribe(() => {
          this.reviewerDocumentFilesList = [];
          const reviewerDocumentList = this.reportSharedService.reviewerDocumentList.filter(a => a.userId === this.userId);
  ...
  openDeleteModal(reviewerDocumentId: string) {
    if (reviewerDocumentId !== undefined) {
      this.apiService.reportsDeleteReviewerDocument(reviewerDocumentId, this.selectedEntityId).subscribe(() => {
        this.reportSharedService.uploadReviewerDocumentAfterDelete(reviewerDocumentId);
   ...
   */
  uploadedDocument() {
    this.reportSharedService.uploadReviewerDocuments(this.reviewerDocumentFilesList, this.userId);
  ...
  downloadFile(reviewerDocumentId: string) {
    this.reportSharedService.downloadReviewerDocument(reviewerDocumentId, this.selectedEntityId);
  }
  ...
  viewFile(reviewerDocumentId: string) {
    this.reportSharedService.openDocumentToView(reviewerDocumentId, this.selectedEntityId);
  }
