...
import { ProcessAC, BASE_PATH } from '../../../../swaggerapi/AngularFiles';
...
@Component({
  ...
  templateUrl: './audit-process-list.component.html'
})
export class AuditProcessListComponent implements OnInit, OnDestroy {
  ...
  auditProcessLabel: string; // Variable for audit process title
  ...
  constructor(
    ...
    private auditProcessService: AuditProcessesService, @Optional() @Inject(BASE_PATH) basePath: string) {
  ...
  loadPageData() {
    ...
    this.getAllAuditProcesses(this.pageNumber, this.itemsPerPage, this.searchValue, this.selectedEntityId);
  }

  /**
   * Get all audit processes under an audtiable entity based on page size and search value
   ...
   */
  getAllAuditProcesses(pageNumber: number, itemsPerPage: number, searchValue: string, selectedEntityId: string) {
    this.auditProcessService.auditProcessesGetAllAuditProcesses(pageNumber, itemsPerPage, searchValue, selectedEntityId)
      .subscribe((result: Pagination<ProcessAC>) => {
        this.loaderService.close();
        this.auditProcessList = result.items;
  ...
  /***
   * Search audit processes basis of name
   * @param event: key press event
   * @param pageNumber: current page number
   * @param itemsPerPage:  no. of items display on per page
   * @param searchValue: search value
   */
  searchAuditProcess(event: KeyboardEvent, pageNumber: number, itemsPerPage: number, searchValue: string) {
    if (event.key === this.stringConstants.enterKeyText || event.key === this.stringConstants.tabKeyText) {
      pageNumber = 1;

      this.loaderService.open();
      this.getAllAuditProcesses(pageNumber, itemsPerPage, searchValue, this.selectedEntityId);
    }
  }


  /**
   * Open audit process add modal for adding audit category
   */
  openAuditProcessAddModal() {
    const initialState = {
      keyboard: true,
      auditProcessObject: {} as ProcessAC,
      callback: (result) => {
        if (result !== undefined) {
    ...
    this.bsModalRef = this.modalService.show(AuditProcessAddComponent,
      Object.assign({ initialState }, { class: 'page-modal audit-team-add' }));
  }

  /**
   * Open audit process edit modal for updating details
   * @param auditProcessId : Seleted audit process Id
   * @param index : Particular index of a list
   */
  openAuditProcessEditModal(auditProcessId: string, index: number) {
    this.loaderService.open();
    this.auditProcessService.auditProcessesGetAuditProcessById(auditProcessId, this.selectedEntityId)
      .subscribe((response) => {
        ...
        const initialState = {
          keyboard: true,
          auditProcessId,
          auditProcessObject: response,
          callback: (result) => {
            if (result !== undefined) {
              // update particular value of the entry
              this.auditProcessList[index] = result;
  ...
  /**
   * Open audit process delete confirmation modal
   * @param auditProcessId : Audit process Id that need to be deleted
   * @param index : Index of the entry you want to delete
   */
  openAuditProcessDeleteModal(auditProcessId: string, index: number) {
    const initialState = {
      ...
      callback: (result) => {
        if (result === this.stringConstants.yes) {
          this.loaderService.open();
          this.auditProcessService.auditProcessesDeleteAuditProcess(auditProcessId, this.selectedEntityId)
            .subscribe(() => {
              this.loaderService.close();
              this.auditProcessList.splice(index, 1);

              if (this.auditProcessList.length === 0 && this.pageNumber > 1) {
  ...
  onItemPerPageChange(pageItem) {
    ...
    if (this.auditProcessList.length === 1) {
      this.pageNumber = this.pageNumber - 1;
    }

    // if current selected item wise page size is greater then start from page first
    if ((this.pageNumber * this.itemsPerPage) > this.auditProcessList.length) {
      this.pageNumber = 1;
    }
  ...
  /**
   * Method for export to excel of audit process
   */
  exportToExcel() {
    const offset = new Date().getTimezoneOffset();

    const url = this.baseUrl + this.stringConstants.exportToExcelAuditProcessApi + this.selectedEntityId + '&timeOffset=' + offset;
